...
MATE_HEALTH_CHECK_PHYSICIAN_ROLE = 'mate.health.check.physician.role'


class MateHealthCheckPhysicianRole(models.Model):
    _name = MATE_HEALTH_CHECK_PHYSICIAN_ROLE
...
class MateHealthCheckPhysician(models.Model):
    _name = 'his.physician'
    _description = 'Health Check Physician'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _inherits = {'res.users': 'user_id'}

    user_id = fields.Many2one('res.users', string='Related User', required=True, ondelete='cascade',
                              help='User-related data of the physician')
    code = fields.Char(string='Physician Code', tracking=True)
    specialty_ids = fields.Many2many(
        PHYSICIAN_SPECIALTY, 'physician_specialty_rel', 'physician_id', 'specialty_id', string='Specialties')
    role_ids = fields.Many2many(
        MATE_HEALTH_CHECK_PHYSICIAN_ROLE, 'physician_role_rel', 'physician_id', 'role_id', string='Roles',
        tracking=True)
    ...
    schedule_ids = fields.One2many('mate.health.check.schedule', 'doctor_id', string='Schedules')
    ...
    def create(self, vals_list):
        ...
        res = super(MateHealthCheckPhysician, self).create(vals_list)
        return res
    ...
    def _compute_is_gp(self):
        ...
        gp_role = self.env.ref('mate_health_check.physician_role_gp', raise_if_not_found=False)
        for rec in self:
    ...
    def _compute_is_specialist(self):
        ...
        specialist_role = self.env.ref('mate_health_check.physician_role_specialist', raise_if_not_found=False)
        for rec in self:
