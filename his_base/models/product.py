...
class ProductProduct(models.Model):
    ...
    def clinic_get_pricelist(self, partner):
        ...
        if clinic_pricelist_id:
            pricelist_id = self.env['product.pricelist'].browse(clinic_pricelist_id)
        elif partner.property_product_pricelist:
            pricelist_id = partner.property_product_pricelist
        return pricelist_id

    def clinic_get_pricelist_price(self, pricelist, quantity, uom_id):
        if pricelist:
            uom = False
            if uom_id:
    ...
    def _clinic_get_partner_price(self, quantity=1, uom_id=False, partner=False):
        pricelist_id = self.clinic_get_pricelist(partner)
        #if any pricelist pass price based on pricelist else default price.
        if pricelist_id:
            price = self.clinic_get_pricelist_price(pricelist_id, quantity, uom_id)
        else:
            price = self.list_price
        return price
    ...
    # def _clinic_get_partner_price_discount(self, quantity=1, uom_id=False, partner=False):
    #     discount = 0
    #     base_price = self.list_price
    #     pricelist_id = self.clinic_get_pricelist(partner)
    #     #if any pricelist pass price based on pricelist else default price.
    #     if pricelist_id:
    #         pricelist_price = self.clinic_get_pricelist_price(pricelist_id, quantity, uom_id)
...
class product_template(models.Model):
    ...
    therapeutic_effect_ids = fields.Many2many('his.therapeutic.effect', 'therapeutic_action_rel', 'therapeutic_effect_id', 'effect_id', string='Therapeutic Effect', help='Therapeutic action')
    ...
    clinic_fixed_price = fields.Float("Fixed Price")
    clinic_min_qty = fields.Integer('Min Quantity')
...
class StockProductionLot(models.Model):
    ...
    def _search_product_qty(self, operator, value):
        valid_record = []
        product_id = self._context.get('clinic_product_id',False)
        domain =[('product_id','=',product_id)]
        if self._context.get('clinic_all_products',False):
            domain =[]
