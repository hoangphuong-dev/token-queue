...
class Appointment(models.Model):
    ...
    def get_procedures_to_invoice(self):
        Procedure = self.env['clinic.patient.procedure']
        for rec in self:
    ...
    def clinic_get_department(self):
        for rec in self:
            ...
            if rec.department_id and rec.department_id.id:
                clinic_department_id = self.env['hr.department'].sudo().search([('id','=',rec.department_id.id)]).id
    ...
    department_id = fields.Many2one('hr.department', ondelete='restrict', 
        domain=lambda self: self.clinic_get_department_domain(), string='Department', tracking=1)
    ...
    clinic_department_id = fields.Many2one('hr.department', compute="clinic_get_department")
    ...
    clinic_weight_name = fields.Char(related="evaluation_id.clinic_weight_name", string='Patient Weight unit of measure label')
    clinic_height_name = fields.Char(related="evaluation_id.clinic_height_name", string='Patient Height unit of measure label')
    clinic_temp_name = fields.Char(related="evaluation_id.clinic_temp_name", string='Patient Temp unit of measure label')
    clinic_spo2_name = fields.Char(related="evaluation_id.clinic_spo2_name", string='Patient SpO2 unit of measure label')
    ...
    appointment_invoice_policy = fields.Selection([('at_end','Invoice in the End'),
        ...
        ('foc','No Invoice')],compute=_clinic_invoice_policy, string="Appointment Invoicing Policy")
    invoice_exempt = fields.Boolean('Invoice Exempt')
    consultation_type = fields.Selection([
        ('consultation','Consultation'),
        ('followup','Follow Up')],'Consultation Type', copy=False)

    diseases_ids = fields.Many2many('his.diseases', 'diseases_appointment_rel', 'diseas_id', 'appointment_id', 'Diseases')
    ...
    responsible_id = fields.Many2one('his.physician', "Responsible Jr. Doctor")
    medical_alert_ids = fields.Many2many('clinic.medical.alert', 'appointment_medical_alert_rel','appointment_id', 'alert_id',
    ...
    allergy_ids = fields.Many2many('clinic.medical.allergy', 'appointment_allergies_rel','appointment_id', 'allergies_id',
    ...
    invoice_count = fields.Integer(compute="_clinic_get_invoice_count", string="#Invoices", groups="account.group_account_invoice")
    procedure_to_invoice_ids = fields.Many2many('clinic.patient.procedure', 'clinic_appointment_procedure_rel', 'appointment_id', 'procedure_id', compute="get_procedures_to_invoice", string="Procedures to Invoice")
    ...
    referred_from_physician_id = fields.Many2one('his.physician', related='referred_from_appointment_id.physician_id', string='Referred from Physician', tracking=1, store=True)
    ...
    referred_to_physician_id = fields.Many2one('his.physician', related='referred_to_appointment_id.physician_id', ondelete='restrict', string='Referred to Physician', tracking=1, store=True)
    ...
    clinic_show_create_invoice = fields.Boolean(compute="get_clinic_show_create_invoice", string="Show Create Invoice Button")
    clinic_show_consumable_create_invoice = fields.Boolean(compute="get_clinic_show_create_invoice", string="Show Consumable Create Invoice Button")
    ...
    patient_procedure_ids = fields.One2many('clinic.patient.procedure', 'appointment_id', 'Patient Procedures')
    ...
    clinic_show_invoice_exempt_request_button = fields.Boolean(compute="_compute_clinic_show_invoice_exempt_request_button")
    ...
    def _compute_clinic_show_invoice_exempt_request_button(self):
        for rec in self:
            rec.clinic_show_invoice_exempt_request_button = not self.env.user.has_group('his_base.group_clinic_invoice_exemption')
    ...
    def get_clinic_show_create_invoice(self):
        for rec in self:
            ...
            rec.clinic_show_consumable_create_invoice = True if ((rec.invoice_id) and 
                                                   rec.state!='done' and 
    ...
    def default_get(self, fields):
        ...
        if self._context.get('clinic_department_type'):
            department = self.env['hr.department'].search([('department_type','=',self._context.get('clinic_department_type'))], limit=1)
            if department:
    ...
    def update_reminder_dates(self):
        for rec in self:
            if fields.Datetime.now() < rec.date:
                reminder_date = rec.date - timedelta(days=int(rec.company_id.clinic_reminder_day)) - timedelta(hours=int(rec.company_id.clinic_reminder_hours))
    ...
    def send_appointment_reminder(self):
        ...
        reminder_appointments = self.sudo().search([('clinic_reminder_sent', '=', False),('state', 'in', ['draft','confirm']),('date','>',fields.Datetime.now()),('reminder_date','<=', date_time_now)])
    ...
    def onchange_department(self):
        ...
        if self.department_id:
            physicians = self.env['his.physician'].search([('department_ids', 'in', self.department_id.id)])
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('his.appointment', sequence_date=seq_date) or _("New")
    ...
    def action_appointment_send(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('his.clinic_appointment_email', raise_if_not_found=False)

        ctx = {
            'default_model': 'his.appointment',
    ...
    def clinic_appointment_common_invoicing(self, invoice_id):
        ...
        if invoice_id:
            for line in data:
                inv_line = self.with_context(clinic_pricelist_id=self.pricelist_id.id).clinic_create_invoice_line(line, invoice_id)
        return data
    ...
    def create_invoice(self):
        ...
        invoice = self.with_context(clinic_context).clinic_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def create_consumed_prod_invoice(self):
        ...
        invoice = self.with_context(pricelist_context).clinic_create_invoice(partner=self.patient_id.partner_id, patient=self.patient_id, product_data=product_data, inv_data=inv_data)
    ...
    def action_refer_doctor(self):
        action = self.env["ir.actions.actions"]._for_xml_id("his.action_appointment")
    ...
    def action_create_evaluation(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("his.action_clinic_patient_evaluation_popup")
        action['domain'] = [('patient_id','=',self.id)]
    ...
    def onchange_physician(self):
        product_id = False
        #Mate: First check configuration on department.
        if self.clinic_department_id:
            #Mate: To avoid portal access error research department here.
            if self.consultation_type=='followup':
                if self.clinic_department_id.followup_service_id:
                    product_id = self.clinic_department_id.followup_service_id.id

            elif self.clinic_department_id.consultation_service_id:
                product_id = self.clinic_department_id.consultation_service_id.id
    ...
    def appointment_confirm(self):
        ...
        if self.patient_id.email and (self.company_id.clinic_auto_appo_confirmation_mail or self._context.get('clinic_online_transaction')):
    ...
    def action_prescription(self):
        action = self.env["ir.actions.actions"]._for_xml_id("his.act_open_his_prescription_order_view")
    ...
    def action_view_treatment(self):
        action = self.env["ir.actions.actions"]._for_xml_id("his.clinic_action_form_hospital_treatment")
        action['context'] = {
    ...
    def cancel_old_appointments(self):
        ...
        if self.env.user.sudo().company_id.clinic_cancel_old_appointment:
            previous_appointments = self.env['his.appointment'].search([('date','<=',yesterday),('state','in',['draft','confirm'])])
...
class StockMove(models.Model):
    ...
    appointment_id = fields.Many2one('his.appointment', string="Appointment", ondelete="restrict")
