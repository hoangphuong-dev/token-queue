...
class ResPartner(models.Model):
    ...
    assignee_ids = fields.Many2many('res.partner','clinic_partner_asignee_relation','partner_id','assigned_partner_id','Assignees', help='Assigned partners for receptionist or doctor etc to see the records')
...
class ResUsers(models.Model):
    ...
    physician_ids = fields.One2many('his.physician', 'user_id', string='Related Physician')
    ...
    def action_create_physician(self):
        ...
        physician = self.env['his.physician'].create({
            'user_id': self.id,
            'name': self.name,
        })
        if portal_user:
