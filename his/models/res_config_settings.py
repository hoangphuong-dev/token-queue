...
class ResConfigSettings(models.TransientModel):
    ...
    clinic_check_appo_payment = fields.Boolean(related='company_id.clinic_check_appo_payment', string="Check Appointment Payment Status before Accepting Request", readonly=False)
    ...
    group_patient_registration_invoicing = fields.Boolean("Patient Registration Invoicing", implied_group='his.group_patient_registration_invoicing')
    group_treatment_invoicing = fields.Boolean("Treatment Invoicing", implied_group='his.group_treatment_invoicing')
    clinic_prescription_qrcode = fields.Boolean(related='company_id.clinic_prescription_qrcode', string="Print Authentication QrCode on Prescription", readonly=False)
    auto_followup_days = fields.Float(related='company_id.auto_followup_days', string='Default Followup on (Days)', readonly=False)
    patient_weight_measure_uom = fields.Char(string='Patient Weight unit of measure', config_parameter='his.clinic_patient_weight_uom')
    patient_height_measure_uom = fields.Char(string='Patient Height unit of measure', config_parameter='his.clinic_patient_height_uom')
    patient_temp_measure_uom = fields.Char(string='Patient Temp unit of measure', config_parameter='his.clinic_patient_temp_uom')
    patient_spo2_measure_uom = fields.Char(string='Patient SpO2 unit of measure', config_parameter='his.clinic_patient_spo2_uom')
    patient_rbs_measure_uom = fields.Char(string='Patient RBS unit of measure', config_parameter='his.clinic_patient_rbs_uom')
    patient_head_circum_measure_uom = fields.Char(string='Patient Head Circumference unit of measure', config_parameter='his.clinic_patient_head_circum_uom')
    clinic_cancel_old_appointment = fields.Boolean(string='Cancel Old Appointment', related='company_id.clinic_cancel_old_appointment', readonly=False)
    clinic_auto_appo_confirmation_mail = fields.Boolean(string="Send Appointment Confirmation Mail", 
        related='company_id.clinic_auto_appo_confirmation_mail', readonly=False)

    clinic_reminder_day = fields.Float(related='company_id.clinic_reminder_day',string="Reminder Days", readonly=False)
    clinic_reminder_hours = fields.Float(related='company_id.clinic_reminder_hours',string="Reminder Hours", readonly=False)
    clinic_flag_days = fields.Integer(related='company_id.clinic_flag_days', string="Warning Flag Days", help="Days to count cancelled appointment", readonly=False)
    clinic_flag_count_limit = fields.Integer(related='company_id.clinic_flag_count_limit', string="Count Limit for Flag", help="Configure number to show alert flag after couting that many cancelled appointments", readonly=False)
