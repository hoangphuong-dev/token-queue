...
class Digest(models.Model):
    ...
    def _compute_kpi_clinic_appointment_total_value(self):
        if not self.env.user.has_group('his_base.group_his_user'):
            raise AccessError(_("Do not have access, skip this data for user's digest email"))
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            appointment = self.env['his.appointment'].search_count([('company_id', '=', company.id), ('date', '>=', start), ('date', '<', end), ('state', 'not in', ['cancel'])])
    ...
    def _compute_kpi_clinic_treatment_total_value(self):
        ...
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            treatment = self.env['his.treatment'].search_count([('company_id', '=', company.id), ('date', '>=', start), ('date', '<', end), ('state', 'not in', ['cancel'])])
    ...
    def _compute_kpi_clinic_procedure_total_value(self):
        ...
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            procedure = self.env['clinic.patient.procedure'].search_count([('company_id', '=', company.id), ('date', '>=', start), ('date', '<', end), ('state', 'not in', ['cancel'])])
    ...
    def _compute_kpi_clinic_evaluation_total_value(self):
        ...
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            evaluation = self.env['clinic.patient.evaluation'].search_count([('company_id', '=', company.id), ('date', '>=', start), ('date', '<', end), ('state', 'not in', ['cancel'])])
    ...
    def _compute_kpi_clinic_patients_total_value(self):
        ...
        for record in self:
            start, end, company = record._get_kpi_compute_parameters()
            patient = self.env['his.patient'].search_count([('company_id', '=', company.id), ('create_date', '>=', start), ('create_date', '<', end)])
            record.kpi_clinic_patients_total_value = patient

    def _compute_kpis_actions(self, company, user):
        res = super(Digest, self)._compute_kpis_actions(company, user)
        res['kpi_clinic_appointment_total'] = 'his.action_appointment&menu_id=%s' % self.env.ref('his.main_menu_appointment').id
        res['kpi_clinic_treatment_total'] = 'his.clinic_action_form_hospital_treatment&menu_id=%s' % self.env.ref('his.main_menu_treatment').id
        res['kpi_clinic_procedure_total'] = 'his.action_clinic_patient_procedure&menu_id=%s' % self.env.ref('his.menu_clinic_patient_procedure_treatment').id
        res['kpi_clinic_evaluation_total'] = 'his.action_clinic_patient_evaluation'
        res['kpi_clinic_patients_total'] = 'his_base.action_patient&menu_id=%s' % self.env.ref('his_base.main_menu_patient').id
        return res