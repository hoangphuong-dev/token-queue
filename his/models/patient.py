...
class MatePatient(models.Model):
    ...
    last_evaluation_id = fields.Many2one("clinic.patient.evaluation", string="Last Evaluation", compute=_get_last_evaluation, readonly=True, store=True)
    ...
    clinic_weight_name = fields.Char(related="last_evaluation_id.clinic_weight_name", string='Patient Weight unit of measure label')
    clinic_height_name = fields.Char(related="last_evaluation_id.clinic_height_name", string='Patient Height unit of measure label')
    clinic_temp_name = fields.Char(related="last_evaluation_id.clinic_temp_name", string='Patient Temp unit of measure label')
    clinic_spo2_name = fields.Char(related="last_evaluation_id.clinic_spo2_name", string='Patient SpO2 unit of measure label')
    clinic_rbs_name = fields.Char(related="last_evaluation_id.clinic_rbs_name", string='Patient RBS unit of measure label')
    ...
    show_cancellation_warning_flag = fields.Boolean(compute='clinic_check_cancellation_flag', string='Show Cancellation Flag')
    clinic_flag_days = fields.Integer(compute='clinic_check_cancellation_flag', string='Flag Days')
    clinic_cancelled_appointments = fields.Integer(compute='clinic_check_cancellation_flag', string='Cancelled Appointments')
    ...
    def show_weight_chart(self):
        action = self.env["ir.actions.actions"]._for_xml_id("his.action_patient_evaluation_graph_1")
    ...
    def create_invoice(self):
        ...
        invoice = self.clinic_create_invoice(partner=self.partner_id, patient=self, product_data=[{'product_id': product_id}], inv_data={'hospital_invoice_type': 'patient'})
    ...
    def action_evaluation(self):
        action = self.env["ir.actions.actions"]._for_xml_id("his.action_clinic_patient_evaluation")
        action['domain'] = [('patient_id','=',self.id)]
        action['context'] = {'default_patient_id': self.id, 'default_physician_id': self.primary_physician_id.id}
        return action
...
class MateFamilyMember(models.Model):
    ...
    related_patient_id = fields.Many2one('his.patient', string='Family Member', help='Family Member Name', required=True)    
