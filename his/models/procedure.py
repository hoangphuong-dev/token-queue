...
class ClinicPatientProcedure(models.Model):
    _name="clinic.patient.procedure"
    ...
    appointment_ids = fields.Many2many('his.appointment', 'clinic_appointment_procedure_rel', 'appointment_id', 'procedure_id', 'Appointments')
    department_id = fields.Many2one('hr.department', ondelete='restrict', domain=lambda self: self.clinic_get_department_domain(), string='Department', tracking=1)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _("New")) == _("New"):
                ...
                vals['name'] = self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('clinic.patient.procedure', sequence_date=seq_date) or _("New")
