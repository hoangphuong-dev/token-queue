...
registryCustomHeaderButton("create_department_header_button", "hr.department.ListViewReconcile.Buttons", function () {
    this.orm.call("his.create.department.wizard", "action_create_department", [], {}).then((result) => {
        this.action.doAction(result);
    });
});

registryCustomHeaderButton("create_package_header_button", "his_health_check_package.ListViewReconcile.Buttons", function () {
    this.orm.call("create.package.wizard", "action_create_package", [], {}).then((result) => {
        this.action.doAction(result);
    });
});

registryCustomHeaderButton("create_group_sequence_header_button", "his_health_check_group_sequence.ListViewReconcile.Buttons", function () {
