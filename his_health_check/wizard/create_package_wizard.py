...
class CreatePackageWizard(models.TransientModel):
    ...
    package_id = fields.Many2one(
        HIS_HEALTH_CHECK_PACKAGE, string='Package', required=True)
    ...
    def action_save_package(self):
        self.env[HIS_HEALTH_CHECK_PACKAGE_LINE].search([('package_id', '=', self.package_id.id)]).unlink()
        self.package_id.write({
            'gender': self.gender,
            'customer_type': self.customer_type.id,
        })
        for line in self.package_line_ids:
            self.env[HIS_HEALTH_CHECK_PACKAGE_LINE].create({
        ...
        return self.env.ref('his_health_check.his_health_check_package_action').read()[0]

    @api.model
    def action_create_package(self):
        action = self.env.ref('his_health_check.action_mate_create_package_wizard').read()[0]
        action['context'] = {
    ...
    def _onchange_package_id(self):
        ...
        if self.package_id:
            package_line = self.env[HIS_HEALTH_CHECK_PACKAGE_LINE].search([('package_id', '=', self.package_id.id)])
